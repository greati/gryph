...
\paragraph{Casting}
\label{sec:casting}
Expression can be explicitly converted from one type to another with the use of the operator \key{@}. For example, in the following code we cast an integer and a float to its string representations before concateneting and printing them:
\begin{lstlisting}[language=Gryph]
i : int = 3;
f : float = 0.3;
